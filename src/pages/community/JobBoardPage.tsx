...
import { fetchFeed, getAvatarColors, getInitials, getRegColorStyle, timeAgoHindi, subscribeToSocialFeed, type SocialPost } from '../../lib/social';
...
export default function JobBoardPage() {
  ...
  const load = useCallback(async () => {
    ...
    let filtered = list;
    if (roleFilter) filtered = filtered.filter(p => p.jobDetails?.role.toLowerCase().includes(roleFilter.toLowerCase()));
    if (locFilter) filtered = filtered.filter(p => p.jobDetails?.location.toLowerCase().includes(locFilter.toLowerCase()));
    setJobs(filtered);
    setLoading(false);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        ...
        {(roleFilter || locFilter) && (
          <button onClick={() => { setRoleFilter(''); setLocFilter(''); }} style={{ height: 38, padding: '0 10px', borderRadius: 10, border: '1.5px solid var(--neutral-200)', background: '#fff', cursor: 'pointer', fontSize: 12, fontFamily: 'inherit', color: 'var(--neutral-600)' }}>✕</button>
        )}
      </div>
      ...
      {!loading && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ fontSize: 12, color: 'var(--neutral-500)', marginBottom: 4, fontWeight: 600 }}>{jobs.length} नौकरियां उपलब्ध हैं</div>
          {jobs.map(post => {
              ...
              <div key={post.id} style={{ background: '#fff', border: '1px solid var(--neutral-200)', borderRadius: 16, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '14px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    {isUrgent && <span style={{ fontSize: 11, padding: '3px 9px', borderRadius: 99, background: '#FEE2E2', color: '#DC2626', fontWeight: 800 }}>URGENT 🔴</span>}
                  </div>
                  {jd?.salary && <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--brand-green)', marginBottom: 10 }}>{jd.salary}</div>}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    ...
                    <span style={{ border: `1.5px solid ${regStyle.border}`, color: regStyle.color, background: '#fff', borderRadius: 4, padding: '1px 6px', fontSize: 10, fontWeight: 500 }}>{post.authorRegNumber}</span>
                  ...
                  {jd?.contactWhatsApp
                    ? <a href={`https://wa.me/91${jd.contactWhatsApp}`} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '10px', borderRadius: 12, background: '#25D366', color: '#fff', textDecoration: 'none', fontSize: 13, fontWeight: 800 }}>📞 WhatsApp पर संपर्क करें</a>
                    : <button onClick={() => navigate(`/captain/community/post/${post.id}`)} style={{ width: '100%', padding: '10px', borderRadius: 12, border: '1.5px solid var(--neutral-200)', background: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', color: 'var(--neutral-700)' }}>विवरण देखें →</button>
                  }
                </div>
              </div>
            );
