...
import {
  getSocialMembers, searchMembers, getAvatarColors, getInitials,
...
export default function CommunityMembersPage() {
  ...
  const allMembers = getSocialMembers().filter(m => m.isActive);
  const totalAll     = allMembers.length;
  const totalCaptain = allMembers.filter(m => m.role === 'captain').length;
  const totalAvail   = allMembers.filter(m => m.badges.includes('available') || (!m.badges.includes('placed') && m.role === 'worker')).length;

  const load = useCallback(() => {
    let base = query ? searchMembers(query) : getSocialMembers().filter(m => m.isActive);
    ...
    setMembers(base);
  }, [query, tab, cityFilter, jobFilter]);

  useEffect(() => { load(); }, [load]);

  const cities = [...new Set(getSocialMembers().filter(m => m.city).map(m => m.city))];

  const TABS: { value: TabFilter; label: string; count: number }[] = [
    { value: 'all',       label: `सभी सदस्य (${totalAll})`,    count: totalAll },
    { value: 'captain',   label: `Captain (${totalCaptain})`,  count: totalCaptain },
    { value: 'available', label: `उपलब्ध (${totalAvail})`,    count: totalAvail },
  ];
    ...
    <div>
      ...
      <div style={{ position: 'relative', marginBottom: 12 }}>
        ...
        <input
          ...
          placeholder="नाम या रजि. क्र. से सर्च करें"
      ...
      {members.length === 0 && (
        <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--neutral-400)' }}>
          ...
          {query ? (
            <>
              <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--neutral-700)', marginBottom: 4 }}>"{query}" के लिए कोई सदस्य नहीं मिला</div>
              <div style={{ fontSize: 13 }}>नाम, रजि. क्र., या शहर से खोजें</div>
            </>
          ) : (
  ...
  return (
    <button onClick={onClick} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, background: '#fff', border: '1px solid var(--neutral-200)', borderRadius: 16, padding: '14px', width: '100%', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {/* Reg number pill — THE KUTUMB SIGNATURE */}
        <div style={{ marginBottom: 7 }}>
          <span style={{
            display: 'inline-block',
            border: `1.5px solid ${regStyle.border}`,
          ...
          {member.role === 'captain' && (
            <span style={{ marginLeft: 6, fontSize: 10, padding: '2px 7px', borderRadius: 99, background: '#FAEEDA', color: '#854F0B', fontWeight: 700 }}>⚡ Captain</span>
          )}
          {member.role === 'ops' && (
            <span style={{ marginLeft: 6, fontSize: 10, padding: '2px 7px', borderRadius: 99, background: '#EEEDFE', color: '#534AB7', fontWeight: 700 }}>✅ Official</span>
          )}
        </div>
