...
function JobCard({ job, onEdit, onToggle }: CardProps) {
  ...
  return (
    <div style={{ background: '#fff', border: `1.5px solid ${job.active ? 'var(--neutral-200)' : 'var(--neutral-100)'}`, borderRadius: 18, padding: 16, marginBottom: 10, opacity: job.active ? 1 : 0.65 }}>
      ...
      {job.description && <div style={{ fontSize: 12, color: 'var(--neutral-500)', marginBottom: 10, lineHeight: 1.5 }}>{job.description}</div>}
...
export default function OpsJobsPage() {
  ...
  const handleEdit = (job: Job) => { setEditing(job); setAdding(false); setForm(job); };
  ...
  return (
    <div>
      ...
      {/* Inline form — no nested component so state changes don't remount it */}
      {(adding || editing) && (
        <div style={{ background: '#fff', border: '1.5px solid var(--brand-green-mid)', borderRadius: 20, padding: 18, marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            ...
            <div><Lbl text="Openings" /><input type="number" min={1} value={form.openings || 1} onChange={e => set('openings', Number(e.target.value))} style={inputStyle} {...focusHandlers} /></div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            ...
            <div><Lbl text="Contact Person" /><input value={form.contact || ''} onChange={e => set('contact', e.target.value)} placeholder="Name / phone" style={inputStyle} {...focusHandlers} /></div>
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            <Lbl text="Additional Details" />
            <textarea value={form.description || ''} onChange={e => set('description', e.target.value)} placeholder="Any extra info for captains..." style={taStyle} {...focusHandlers} />
          </div>
      ...
      {active.length === 0 && !adding && (
        <div style={{ textAlign: 'center', padding: '48px 24px', background: '#fff', borderRadius: 18, border: '1.5px dashed var(--neutral-200)' }}>
          ...
          <button onClick={() => setAdding(true)} style={{ padding: '10px 20px', borderRadius: 12, border: 'none', background: 'var(--brand-green-mid)', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>+ Post First Job</button>
        </div>
      )}
