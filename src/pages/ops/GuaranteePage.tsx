...
type GuaranteeItem = ReturnType<typeof getGuaranteeAlerts>[0];
type SLAItem = ReturnType<typeof getSLAViolations>[0];

function daysBar(daysLeft: number) {
  const pct   = Math.max(0, Math.min(((30 - daysLeft) / 30) * 100, 100));
...
function CandidateRow({ c, onReplace }: { c: GuaranteeItem; onReplace: (id: string, name: string) => void }) {
  ...
  return (
    <div style={{ background:'#fff', border:`1.5px solid ${color === 'var(--danger)' ? 'rgba(220,38,38,0.2)' : color === 'var(--warning)' ? 'rgba(217,119,6,0.2)' : 'var(--neutral-200)'}`, borderRadius:16, padding:14, marginBottom:10 }}>
      ...
      <div style={{ display:'flex', gap:8 }}>
        <a href={`tel:${c.mobile}`} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:6, background:'rgba(46,168,106,0.1)', border:'1px solid rgba(46,168,106,0.2)', borderRadius:10, padding:'9px', fontSize:12, fontWeight:700, color:'var(--brand-green)', textDecoration:'none' }}>📞 Call</a>
        <a href={`https://wa.me/91${c.mobile}?text=${encodeURIComponent(`Hi ${c.name}! Switch टीम से हूँ — check-in करना था। कैसा चल रहा है नया काम? 😊`)}`} target="_blank" rel="noopener noreferrer" style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:6, background:'linear-gradient(135deg,#25D366,#128C7E)', borderRadius:10, padding:'9px', fontSize:12, fontWeight:700, color:'#fff', textDecoration:'none' }}>💬 WhatsApp</a>
        {!c.replacementNeeded && c.daysLeft >= 0 && c.daysLeft <= 7 && (
          <button onClick={() => onReplace(c.id, c.name)} style={{ flex:1, background:'var(--danger-light)', border:'1px solid rgba(220,38,38,0.2)', borderRadius:10, padding:'9px', fontSize:12, fontWeight:700, color:'var(--danger)', cursor:'pointer', fontFamily:'inherit' }}>Replace</button>
        )}
        {c.replacementNeeded && (
...
export default function GuaranteePage() {
  const [alerts, setAlerts] = useState<GuaranteeItem[]>([]);
    ...
    <div>
      ...
      {tab === 'guarantee' && (
        <>
          ...
          {critical.map(c => <CandidateRow key={c.id} c={c} onReplace={handleReplacement} />)}
          <GroupLabel label="Warning (4–7 days)" count={warning7.length} color="var(--warning)" />
          {warning7.map(c => <CandidateRow key={c.id} c={c} onReplace={handleReplacement} />)}
          <GroupLabel label="Active (8–30 days)" count={active30.length} color="var(--brand-green)" />
          {active30.map(c => <CandidateRow key={c.id} c={c} onReplace={handleReplacement} />)}
          <GroupLabel label="Expired" count={expired.length} color="#9CA3AF" />
          {expired.map(c => <CandidateRow key={c.id} c={c} onReplace={handleReplacement} />)}
